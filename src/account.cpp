...
void AccountSystem::add_user(char20 curname, char64 username, Taccountinfo info, bool fir){
    if(fir){
        int cou = accountMemory_.number;
        account_.insert(poser(username, cou));
        accountMemory_.insert(pointer(cou, info));
        std::cout << '0' << std::endl;
    }
    else{
        ...
        pointer ptr = accountMemory_.only(pointer(now.value, Taccountinfo()));
        int level = get<3>(ptr.value);
        if(level <= get<3>(info)){
            std::cout << "-1" << std::endl;
            return;
        }
...
void AccountSystem::login(char64 username, char30 password){
    ...
    pointer ptr = accountMemory_.only(pointer(now.value, Taccountinfo()));
    if(get<0>(ptr.value) != password){
        //std::cout << "wrong password" << std::endl;
        //std::cout << get<0>(ptr.value) << "|||" << password << std::endl;
        std::cout << "-1" << std::endl;
        return;
    }
    stack_.insert(poser(username, ptr.key));
    std::cout << "0" << std::endl;
    return;
}
...
void AccountSystem::query_profile(char64 curname, char64 username){
    ...
    pointer mcp = accountMemory_.only(pointer(cp.value, Taccountinfo())),
        mup = accountMemory_.only(pointer(up.value, Taccountinfo()));
    if(cp.value != up.value && get<3>(mcp.value) <= get<3>(mup.value)){
        std::cout << "-1" << std::endl;
        return;
    }
    std::cout << username << ' ' << get<1>(mup.value) << ' ' << 
        get<2>(mup.value) << ' ' << get<3>(mup.value) << ' ' << std::endl;
    return;
}
...
void AccountSystem::modify_profile(char64 curname, char64 username, Taccountinfo info){
    ...
    if(get<3>(mcp.value) <= get<3>(info)){
        std::cout << "-1" << std::endl;
        return;
    }
    pointer mem = mup;
    if(auto v = get<0>(info); v != char30()){
        get<0>(mup.value) = get<0>(info);
    }
    if(auto v = get<1>(info); v != personName()){
        get<1>(mup.value) = get<1>(info);
    }
    if(auto v = get<2>(info); v != char30()){
        get<2>(mup.value) = get<2>(info);
    }
    if(auto v = get<3>(info); v != -1){
        get<3>(mup.value) = get<3>(info);
    }
    accountMemory_.remove(mem), accountMemory_.insert(mup);
    std::cout << username << ' ' << get<1>(mup.value) << ' ' << 
        get<2>(mup.value) << ' ' << get<3>(mup.value) << ' ' << std::endl;
    return;
}
